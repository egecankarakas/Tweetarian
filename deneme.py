...
conn = sqlite3.connect('twitter10.db')

c= conn.cursor()
c.executescript("""
    DROP TABLE IF EXISTS twitter3;
    CREATE TABLE twitter3(created_at ,id PRIMARY KEY , text, source , lang,
    ...
    rt_status_retweet_count, rt_status_lang, countAvea, countVodafone, countTurkcell);
    """)
...
class StdOutListener(StreamListener):
    ...
    def on_data(self, line):
        ...
        if line.startswith('{'):
            ...
            if "avea" in text.lower():
                numAvea = 1

            if "vodafone" in text.lower():
                numVodafone = 1

            if "turkcell" in text.lower():
                numTurkcell = 1

        c.execute("INSERT INTO twitter3 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                  (created_at ,id , text, source , lang,
                    ...
                    rt_status_user_id, rt_status_user_screen_name, rt_status_user_location, rt_status_user_followers_count,
                    rt_status_user_friends_count,rt_status_user_created_at, rt_status_user_lang, rt_status_user_time_zone,
                    rt_status_retweet_count, rt_status_lang, numAvea, numVodafone, numTurkcell)
                  )
...
if __name__ == '__main__':
    ...
    stream.filter(track=['Turkcell','Avea','Vodafone'])